...
string cword(int lvl){ //function to choose word according to level
    ...
    switch (lvl){//select word on basis of output
        case(1):
        word=
        dictionary1[rand()%30];
        break;
...
int menu(int j,int a,int b,int signal){
    char ca,cb;
    int p=0;
    string outputs[5]={"\033[1;31m Continue\033[0;32m\nStart New Game\nQuit\n","Continue\n\033[1;31m Start New Game\033[0;32m\nQuit\n","Continue\nStart New Game\n\033[1;31m Quit\033[0;32m\n","\033[1;31m Start\033[0;32m\nQuit\n","Start\n\033[1;31m Quit\033[0;32m\n"};//different utputs highlighted
    do{
    if(signal==1){
    system("cls");
    ...
    if(p!=0&&signal==0){
    cout << "\033[F\033[K"; //remove the options while in game
    cout << "\033[F\033[K"; 
    cout << "\033[F\033[K";  
    }
    cout << "\033[32m";//change color of output
    cout<<outputs[j];
    cout << "\033[37m";//restore color
    p++;
    ca=getch();
    if(ca==char(224)){
        cb=getch();
...
int hintmenu(){ 
    ...
    do{
        if(c2!=0){
            cout <<"\033[F\033[K";//remove prev 3 lines but not when initially run
            cout <<"\033[F\033[K";
            cout <<"\033[F\033[K";
        }
        cout<<"\nWould you like a hint : "<<endl;//hint prompt
        cout << "\033[32m";
        cout<<choice[p];
        cout << "\033[37m";
        a=getch();
        if(a==char(224)){
            b=getch();
            if(b=='M'&&p!=1){//move accross options
                p++;
                Beep(500,50);
            }
            if(b=='K'&&p!=0){
        ...
        c2++;
    }while(a != char(13));//end loop on pressing of eenter
    Beep(1500,500);
    return p;
}
...
bool endgame(int wgl,string word,string gword,int cg,int lvl,int &win,int &loss){ //checks to see if game meets conditions to end game 
...
void hint(int wgl,string &gword,string word,int &hintnum){//function to give hint 
    ...
    for(int i1=0;i1<word.length();i1++){
        for(int i2=i1+1;i2<word.length();i2++){
            if(word[i1]==word[i2] && gword[i2]=='_'){//repetitive alphabet that still isnt guessesd isnt counted
                count--;
                i2=word.length();
            }
        }
        ...
        if(wgl<=2){//condition to offer hint
            ...
            if(p==0){//action
                ...
                for(int i=0;i<gword.length();i++){
                    if(gword[i]=='_'){
                        gword[i]=word[i];
                        for(int k=0;k<word.length();k++){//making sure hint is given at all instances
                            if(word[k]==word[i]){
                                gword[k]=word[k];
                            }
                        }
